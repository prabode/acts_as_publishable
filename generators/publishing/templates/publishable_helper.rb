...
module PublishableHelper
  ...
  def publishing_status_tag(model, authentication_token)
    if model.published?
      image_tag("<%=@image_prepend %>publishing/published_small.png")
    elsif model.ready?
      image_tag("<%=@image_prepend %>publishing/ready_small.png")
    elsif model.archived?
      image_tag("<%=@image_prepend %>publishing/archived_small.png")
    else
...
.tooltip {
position: absolute;
background-image: url(/images/<%=@image_prepend %>publishing/arrow.png);
...
<span id="#{model.class.name}_#{model.id}_tooltip_link">#{image_tag("<%=@image_prepend %>publishing/not_ready_small.png",:border=>"0",:style=>'cursor:help')}</span>
...
def publishing_actions_tag(model)
  render :partial => '<%=@view_prepend %>publishing/publishing_status', :locals => { :model => model }
end
